...
/**
 * Sprint 11 — Part 1: האזנה בזמן אמת לרשימת הפרסים (כמו fbWatchShopState) — כך שפרס
 * שהמורה מוסיפה בזמן שמסך החנות של התלמיד/ה כבר פתוח מופיע מיד, בלי לצאת ולהיכנס
 * מחדש. activeOnly מסונן בצד הלקוח בכל snapshot, בדיוק כמו ב-fbLoadRewards.
 * מחזירה פונקציית unsubscribe.
 */
function fbWatchRewards(clubId, callback, { activeOnly = false } = {}) {
  ...
  return _rewardsRef(clubId).onSnapshot(
    ...
    e => console.warn('[firebase-shop] fbWatchRewards error:', e.message)
  );
}
...
async function evaluateGoalProgress(clubId) {
  ...
  try {
    ...
    if (!shop) return;
    if (shop.state === 'GOAL_REACHED_PENDING_SHOP') {
      await fbOpenShopForVoting(clubId);
      return;
    }
    if (shop.state !== 'browsing' || !shop.activeCycleId) return;
...
/**
 * Sprint 11 — Part 2: לוג אבחוני מלא בכל כישלון הצבעה — אותה תבנית בדיוק כמו
 * _logReadingSessionSaveFailure ב-firebase-clubs.js — כדי שכישלון אף פעם לא ייעלם בלי עקבות.
 */
function _logVoteCastFailure(clubId, voteId, userId, e) {
  const authUid = (typeof firebase !== 'undefined' && firebase.auth().currentUser)
    ? firebase.auth().currentUser.uid : null;
  console.warn(
    '[firebase-shop] fbCastVote FAILED',
    {
      code: e?.code, message: e?.message,
      clubId, voteId, userId, authUid, uidMismatch: authUid !== null && authUid !== userId,
...
/**
 * מצביע/ה — יצירה או עדכון (החלפת הצבעה כל עוד ההצבעה פתוחה — ר' firestore.rules).
 * בכל כישלון: מתעד לוג מלא, ומסווגת את הסיבה האמיתית (לא מניחה "כבר הצבעת" כברירת
 * מחדל — permission-denied יכול לנבוע גם מסגירת הצבעה וגם מאי-התאמת זהות).
 */
async function fbCastVote(clubId, voteId, userId, rewardId) {
  ...
  try {
    ...
    _logVoteCastFailure(clubId, voteId, userId, e);
    let reason = 'error';
    if (e.code === 'permission-denied') {
      try {
        ...
        if (!vote || vote.status !== 'open') {
          ...
          const authUid = (typeof firebase !== 'undefined' && firebase.auth().currentUser)
            ? firebase.auth().currentUser.uid : null;
          reason = (authUid && authUid !== userId) ? 'identity-mismatch' : 'permission-denied';
        }
      } catch (e2) {
...
/**
 * מורה סוגרת הצבעה: סופרת קולות, קובעת הצעת-מנצח/ת או תיקו.
 * תיקו → יוצרת סיבוב הכרעה (runoff) עם המועמדים המעורבים בלבד; המצב נשאר voting_open (ללא שינוי).
 * ניצחון ברור → סוגרת את ההצבעה (voting_closed), אבל (Sprint 10 — Part 3) לא נועלת winnerRewardId
 * מיד: שומרת אותו כ-suggestedWinnerRewardId (ברירת המחדל המוצגת למורה), וממתינה לאישור מפורש דרך
 * fbConfirmVoteWinner — כך שהמורה תמיד יכולה לבחור פרס אחר לפני שהבחירה ננעלת.
 */
  ...
  try {
    ...
    if (result.isTie) {
      await _votesRef(clubId).doc(voteId).update({
        ...
        winnerRewardId: null, suggestedWinnerRewardId: null, tiedRewardIds: result.leaders,
      });
      ...
      await runoffRef.set({
        ...
        status: 'open', openedAt: _now(), closedAt: null,
        tally: null, winnerRewardId: null, suggestedWinnerRewardId: null, tiedRewardIds: null,
      });
    ...
    await _votesRef(clubId).doc(voteId).update({
      status: 'closed', closedAt: _now(), tally: result.counts,
      winnerRewardId: null, suggestedWinnerRewardId: result.winnerRewardId, tiedRewardIds: null,
    });
    await _shopRef(clubId).set({ state: 'voting_closed', updatedAt: _now() }, { merge: true });
    return { ok: true, tie: false, suggestedWinnerRewardId: result.winnerRewardId };
  } catch (e) {
...
/**
 * Sprint 10 — Part 3: המורה נועלת את הזוכה הסופי (ברירת המחדל: suggestedWinnerRewardId,
 * אבל אפשר לבחור כל פרס אחר מרשימת האפשרויות). דורשת שההצבעה סגורה וטרם ננעלה.
 * אחרי הנעילה, גם שומרת shop.lastWinner (Part 4) — כדי שהזוכה יישאר גלוי לתלמידים
 * גם אחרי שהחנות חוזרת ל-'browsing', עד שמחזור היעד הבא יתחיל.
 */
async function fbConfirmVoteWinner(clubId, voteId, rewardId) {
  if (!_db() || !clubId || !voteId || !rewardId) return { ok: false, reason: 'missing-data' };
  try {
    const vote = await fbLoadVote(clubId, voteId);
    if (!vote || vote.status !== 'closed' || vote.winnerRewardId) return { ok: false, reason: 'wrong-state' };
    const winner = (vote.rewardOptions || []).find(o => o.rewardId === rewardId);
    if (!winner) return { ok: false, reason: 'reward-missing' };

    await _votesRef(clubId).doc(voteId).update({ winnerRewardId: rewardId });
    await _shopRef(clubId).set({
      lastWinner: {
        rewardId: winner.rewardId, name: winner.name, emoji: winner.emoji || '🎁',
        imageUrl: winner.imageUrl || null, wonAt: _now(),
      },
      updatedAt: _now(),
    }, { merge: true });
    return { ok: true };
  } catch (e) {
    console.warn('[firebase-shop] fbConfirmVoteWinner error:', e.message);
    return { ok: false, reason: 'error' };
  }
}
...
function _startNextGoalCycleTx(tx, clubId, oldCycle, lifetimeSpentAtCycleStart, nextGoalTarget, purchaseId, now) {
  ...
  tx.set(_shopRef(clubId), {
    ...
    // Sprint 10 — Part 4: הזוכה הקודם נשאר גלוי לתלמידים לאורך כל המחזור החדש, ומתנקה
    // (merge:true לא מוחק שדות שלא מוזכרים — לכן צריך null מפורש) בדיוק כשמחזור חדש מתחיל.
    lastWinner: null,
  }, { merge: true });
