...
const same=v=>{if(!v)return false;const d=new Date(v),n=new Date();return !isNaN(d)&&d.toDateString()===n.toDateString()};
function avatarValue(m){return m?.emoji||m?.avatar||m?.profile?.avatar||m?.cachedProfile?.avatar||'📚'}
function av(m){const v=String(avatarValue(m)||'📚');if(/^https?:\/\//i.test(v)||/^data:image\//i.test(v)||/^blob:/i.test(v))return `<img class="reader-avatar-img" src="${esc(v)}" alt="" referrerpolicy="no-referrer">`;return `<span class="reader-avatar-emoji">${esc(v)}</span>`}
const tot=m=>Math.round(m?.cachedStats?.totalMinutes||0),day=m=>Math.round(m?.cachedStats?.todayMinutes??m?.cachedStats?.minutesToday??m?.cachedStats?.lastSessionMinutes??0);
function demo(mode,ms){const a=(ms||[]).filter(m=>m.status!=='left');if(mode===T)return '<div class="bdp-demo tree"><b class="children-see">ככה הילדים יראו את זה:</b><div class="big-tree">🌳</div><strong>העץ והיעד הכיתתי</strong><small>בלי שמות ובלי נתוני ילדים</small></div>';if(mode===D){const l=a.filter(m=>same(m.cachedStats?.lastReadAt)).sort((x,y)=>String(x.name||'').localeCompare(String(y.name||''),'he'));return `<div class="bdp-demo"><b class="children-see">ככה הילדים יראו את זה:</b><h4>קראו היום:</h4><div class="floaters">${l.length?l.map(m=>`<span>${av(m)} <b>${esc(m.name||'קורא/ת')}</b> · ${day(m)} דק׳</span>`).join(''):'<i>עוד לא קראו היום 🌱</i>'}</div><div class="big-tree">🌳</div></div>`;}const s=[...a].sort((x,y)=>tot(y)-tot(x));return `<div class="bdp-demo board"><b class="children-see">ככה הילדים יראו את זה:</b><h4>🏆 מי קרא הכי הרבה</h4>${s.slice(0,5).map((m,i)=>`<div class="rank-row"><b>${i+1}.</b><span class="rank-person">${av(m)} ${esc(m.name||'קורא/ת')}</span><strong>${tot(m)} דק׳</strong></div>`).join('')||'<i>אין עדיין נתוני קריאה</i>'}</div>`;}
function local(el,m){el.querySelectorAll('.bdp-option').forEach(b=>b.classList.toggle('active',b.dataset.mode===m));el.querySelectorAll('.bdp-preview').forEach(p=>p.classList.toggle('open',p.dataset.preview===m));}
async function save(id,m){if(typeof fbSaveClub==='function'){const c=typeof fbLoadClub==='function'?await fbLoadClub(id):null;await fbSaveClub(id,{settings:{...(c?.settings||{}),progressDisplay:m}})}else if(typeof orig==='function'&&m!==D)await orig(id,m)}window.setProgressDisplayMode=save;
function picker(id,m,ms){const e=document.createElement('div');e.className='booki-display-picker';e.innerHTML=`<div class="bdp-title">👀 מה הילדים יראו ליד העץ הכיתתי?</div><button type="button" class="bdp-option" data-mode="${B}"><strong>🏆 תחרותי — מי קרא הכי הרבה</strong><span>דירוג לפי סך זמן הקריאה עד היום.</span></button><div class="bdp-preview" data-preview="${B}">${demo(B,ms)}</div><button type="button" class="bdp-option" data-mode="${D}"><strong>📚 קראו היום <em>ללא תחרות</em></strong><span>רק מי שקרא היום, עם אייקון, שם ודקות. ללא דירוג.</span></button><div class="bdp-preview" data-preview="${D}">${demo(D,ms)}</div><button type="button" class="bdp-option" data-mode="${T}"><strong>🌳 העץ בלבד</strong><span>היעד הכיתתי בלבד, בלי שמות ובלי נתוני ילדים.</span></button><div class="bdp-preview" data-preview="${T}">${demo(T,ms)}</div><p class="bdp-note">💡 הבחירה משנה את תצוגת הילדים בלבד.</p><div class="bdp-save"></div>`;local(e,m);e.onclick=x=>{const b=x.target.closest('.bdp-option');if(!b)return;x.preventDefault();x.stopPropagation();const md=b.dataset.mode;local(e,md);const st=e.querySelector('.bdp-save');st.textContent='שומר…';save(id,md).then(()=>{st.textContent='✓ נשמר';setTimeout(()=>st.textContent='',1000)}).catch(()=>st.textContent='לא נשמר — נסי שוב')};return e;}
